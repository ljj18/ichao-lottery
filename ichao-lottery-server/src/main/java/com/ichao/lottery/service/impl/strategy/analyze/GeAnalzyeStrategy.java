 ...
 */
@Service
public class GeAnalzyeStrategy implements IAnalyzeStrategy {

    @Override
    public void doAnalyze(P3Dto pre, P3Dto curr, AnalyzeResultDto respDto) {
        GeAnalyzeResult ba = respDto.getGe();
        if (pre != null) {
            if (curr.getGe() == pre.getGe()) {
                ba.setRepeat(ba.getRepeat() + 1);
            } else if (curr.getGe().getValue() > pre.getGe().getValue()) {
