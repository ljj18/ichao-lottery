 ...
 */
@Service
public class ShiAnalzyeStrategy implements IAnalyzeStrategy {

    @Override
    public void doAnalyze(P3Dto pre, P3Dto curr, AnalyzeResultDto respDto) {
        ShiAnalyzeResult ba = respDto.getShi();
        if (pre != null) {
            if (curr.getShi() == pre.getShi()) {
                ba.setRepeat(ba.getRepeat() + 1);
            } else if (curr.getShi().getValue() > pre.getShi().getValue()) {
